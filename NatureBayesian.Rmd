...
data <- read.csv(file = 'H:/NLMT/DataForR_20191216.csv')
data
```
...
TTEPPoS <- function(n, Y0, T0, r, a, b, crit_val, nsims, pthresh , xfu) {
  ...
  for(i in 1:nsims){
    ...
    #time_ev <- rexp(n , rate = log(2)/TM)
    
    #data <- data.frame(time_fu, time_ev)
    #data <- mutate(data, Y = ifelse(time_ev < time_fu, 1, 0), Times =  ifelse(time_ev < time_fu, time_ev, time_fu))
    #data <- select(data, Times, Y)
...
write.csv(data, file = "H:/NLMT/OutputFromR_20191216.csv")
```
...
data <- read.csv(file = 'H:/NLMT/SmokeSquamusForR_20191216.csv')
data
```
...
write.csv(data, file = "H:/NLMT/SmokeSquamousFromR_20191216.csv")
